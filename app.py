...
def knowledge():
    if request.method == 'POST':
        ...
        if operation == 'discover':
            ...
            if images:
                base64_images = [convert_to_jpeg(image) for image in images]
                if all(base64_images):
                    prompt = f"Give the name, place of origin, mostly found in areas, life span, medicinal value of the tree/plant/leaves/flower etc. present in the picture(s) in detail in {language}"
                    response = request_openai_api(base64_images, prompt)
                    if response:
                        response_html = markdown.markdown(response)
                        return render_template('knowledge.html', discover_response=response_html, discover_images=base64_images)
            ...
            if user_prompt:
                prompt = f"{user_prompt} in {language}"  # Include the language in the prompt
                response = request_openai_api(base64_images, prompt)
                if response:
                    response_html = markdown.markdown(response)
